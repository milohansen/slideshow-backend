...
import { countVariantsForBlob, getActivePickerSession, getAllDevices, getBlob, getDeviceVariantsForBlob, getSourcesForBlob } from "../db/helpers-firestore.ts";
...
ui.get("/images", async (c) => {
  try {
    ...
    const blobsSnapshot = await db.collection(Collections.BLOBS).orderBy("created_at", "desc").limit(100).get();

    // Fetch additional metadata for each blob in parallel
    const imagesWithColors = await Promise.all(
      blobsSnapshot.docs.map(async (doc) => {
        const data = doc.data();
        ...
        return {
          ...
          colors: palette
            ? {
                ...
                secondary: palette[1] || "#000000",
                tertiary: palette[2] || "#000000",
              }
            : null,
        };
      })
...
// Images page - with full metadata from Firestore
ui.get("/images/:hash", async (c) => {
  try {
    const blobHash = c.req.param("hash");

    const data = await getBlob(c.req.param("hash"));
    ...
    return c.html(<Image image={imageData} />);
  } catch (error) {
    console.error("Error loading images page:", error);
    return c.html(<Images images={[]} error="Failed to load images. Check Firestore security rules and authentication." />);
  }
...
ui.get("/queues", async (c) => {
  try {
    ...
    const queues = await Promise.all(
      queueStatesSnapshot.docs.map(async (doc) => {
        const data = doc.data();
...
ui.post("/images/:id/retry", async (c) => {
  ...
  try {
    ...
    const snapshot = await db.collection(Collections.SOURCES).where("blob_hash", "==", imageId).where("status", "==", "failed").limit(1).get();
