...
/**
 * Create a blended transition between two images
 * Useful for slideshow transitions
 */
export async function blendImages(
  image1Path: string,
  image2Path: string,
  outputPath: string,
  blendPercentage: number = 50
): Promise<string> {
  await ensureDir(join(outputPath, ".."));
  ...
  if (code !== 0) {
    const error = new TextDecoder().decode(stderr);
    throw new Error(`Failed to blend images: ${error}`);
  }
...
export async function applyVignette(
  ...
  // Vignette formula: intensity controls how dark the edges get
  const vignetteFormula = `1-${intensity}*((i-w/2)^2+(j-h/2)^2)/(w*h/4)`;
  ...
  if (code !== 0) {
    const error = new TextDecoder().decode(stderr);
    throw new Error(`Failed to apply vignette: ${error}`);
  }
