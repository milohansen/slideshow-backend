...
import type { Blob, Source, DeviceVariant, Device, DeviceQueueState, AuthSession, PickerSession, FailedTask } from "./types.ts";
 ...
 */
export async function updateBlobColors(hash: string, colorPalette: string, colorSource: string): Promise<void> {
  const db = getFirestore();
 ...
 */
export async function updateBlob(hash: string, updates: Partial<Blob>): Promise<void> {
  const db = getFirestore();
 ...
 * Get blobs by orientation
 */
export async function listUnanalyzedBlobs(): Promise<Blob[]> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.BLOBS).get();

  const unanalyzedBlobs: Blob[] = [];
  for (const doc of snapshot.docs) {
    const blob = doc.data() as Blob;
    if (!blob.analyzed_at) {
      unanalyzedBlobs.push(blob);
    }
  }
 ...
 */
export async function updateSourceStatus(id: string, status: Source["status"], statusMessage?: string, blobHash?: string): Promise<void> {
  const db = getFirestore();
 ...
 */
export async function getSourcesByStatus(status: Source["status"], limit?: number): Promise<Source[]> {
  const db = getFirestore();
...
export async function getSourcesForBlob(blobHash: string): Promise<Source[]> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.SOURCES).where("blob_hash", "==", blobHash).get();
...
export async function hasExistingSource(externalId: string): Promise<boolean> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.SOURCES).where("external_id", "==", externalId).get();
...
export async function countSourcesByStatus(): Promise<Record<string, number>> {
  ...
  await Promise.all(
    statuses.map(async (status) => {
      const snapshot = await db.collection(Collections.SOURCES).where("status", "==", status).count().get();
 ...
 */
export async function createDeviceVariant(variant: Omit<DeviceVariant, "id" | "processed_at">): Promise<string> {
  const db = getFirestore();
 ...
 */
export async function getDeviceVariant(blobHash: string, width: number, height: number): Promise<DeviceVariant | undefined> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.DEVICE_VARIANTS).where("blob_hash", "==", blobHash).where("width", "==", width).where("height", "==", height).limit(1).get();
...
export async function getDeviceVariantsForBlob(blobHash: string): Promise<DeviceVariant[]> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.DEVICE_VARIANTS).where("blob_hash", "==", blobHash).get();

  return snapshot.docs.map((doc) => doc.data() as DeviceVariant);
}

export async function cleanDuplicateDeviceVariants(): Promise<DeviceVariant[]> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.DEVICE_VARIANTS).get();

  const dupes: DeviceVariant[] = [];
  const seen = new Set<string>();

  for (const doc of snapshot.docs) {
    const data = doc.data() as DeviceVariant;
    const path = data.storage_path;
    if (seen.has(path)) {
      dupes.push(data);
      await db.collection(Collections.DEVICE_VARIANTS).doc(doc.id).delete();
    } else {
      seen.add(path);
    }
  }
...
export async function countVariantsForBlob(blobHash: string): Promise<number> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.DEVICE_VARIANTS).where("blob_hash", "==", blobHash).count().get();
...
export async function deleteVariantsForBlob(blobHash: string): Promise<void> {
  const db = getFirestore();
  const variants = await db.collection(Collections.DEVICE_VARIANTS).where("blob_hash", "==", blobHash).get();
 ...
 */
export async function getActiveDeviceDimensions(): Promise<Array<{ width: number; height: number; orientation: string }>> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.DEVICE_VARIANTS).get();
 ...
 */
export async function deviceVariantExists(blobHash: string, width: number, height: number): Promise<boolean> {
 ...
 */
export async function getDeviceQueueState(deviceId: string): Promise<DeviceQueueState | undefined> {
  const db = getFirestore();
 ...
 */
export async function updateDeviceQueueState(deviceId: string, queueData: string, currentIndex: number): Promise<void> {
  const db = getFirestore();
 ...
 */
export async function getAuthSessionByUserId(userId: string): Promise<AuthSession | undefined> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.AUTH_SESSIONS).where("user_id", "==", userId).limit(1).get();
...
export async function getActiveAuthSession(): Promise<AuthSession | undefined> {
  ...
  const snapshot = await db.collection(Collections.AUTH_SESSIONS).where("token_expiry", ">", now).orderBy("token_expiry", "desc").limit(1).get();
...
export async function deleteAuthSession(id: string): Promise<void> {
  ...
  const pickerSessions = await db.collection(Collections.PICKER_SESSIONS).where("user_id", "==", id).get();
 ...
 */
export async function createPickerSession(session: Omit<PickerSession, "id" | "created_at">): Promise<string> {
  const db = getFirestore();
  const id = crypto.randomUUID();

  await db
    .collection(Collections.PICKER_SESSIONS)
    .doc(id)
    .set({
      ...session,
      id,
 ...
 */
export async function updatePickerSession(id: string, updates: Partial<PickerSession>): Promise<void> {
  const db = getFirestore();
...
export async function deleteExpiredPickerSessions(): Promise<void> {
  ...
  const snapshot = await db.collection(Collections.PICKER_SESSIONS).where("expire_time", "<", now).get();
 ...
 */
export async function createFailedTask(task: Omit<FailedTask, "id" | "created_at">): Promise<string> {
  const db = getFirestore();
  const id = crypto.randomUUID();

  await db
    .collection(Collections.FAILED_TASKS)
    .doc(id)
    .set({
      ...task,
      id,
...
export async function getAllFailedTasks(): Promise<FailedTask[]> {
  const db = getFirestore();
  const snapshot = await db.collection(Collections.FAILED_TASKS).orderBy("created_at", "desc").get();
...
export async function deleteBlob(hash: string): Promise<void> {
  ...
  const variants = await db.collection(Collections.DEVICE_VARIANTS).where("blob_hash", "==", hash).get();
  ...
  const sources = await db.collection(Collections.SOURCES).where("blob_hash", "==", hash).get();
